...
node(:html) { |entry| "<iframe src=\"#{embed_account_stream_entry_url(entry.account, entry, (@extra_params || {}))}\" style=\"width: 100%; overflow: hidden\" frameborder=\"0\" width=\"#{@width}\" height=\"#{@height}\" scrolling=\"no\"></iframe>" }
node(:width) { @width }
node(:height) { @height }
