...
export function updateNotifications(notification, intlMessages, intlLocale) {
  return (dispatch, getState) => {
    ...
    if (typeof window.Notification !== 'undefined' && showAlert) {
      const account = notification.account ? notification.account : notification.status.account;
      const title = new IntlMessageFormat(intlMessages[`notification.${notification.type}`], intlLocale).format({ name: account.display_name.length > 0 ? account.display_name : account.username });
      const body  = (notification.status && notification.status.spoiler_text.length > 0) ? notification.status.spoiler_text : unescapeHTML(notification.status ? notification.status.content : '');

      const notify = new Notification(title, { body, icon: account.avatar, tag: notification.id });
